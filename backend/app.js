...
io.on("connection", (socket) => {
  ...
  async function sendMail (email, name, age){
  ...
  async function createNoti (customerBirthday, age, customer, _id, nameEm) {
    await Notification.create({
      title: "Sinh nhật",
      content: `Ngày mai ${customerBirthday.date}/${customerBirthday.month + 1} là sinh nhật thứ ${age} của khách hàng "${customer.name}"`,
      recipient: nameEm,
      sender: "",
      isRead: false,
      idRecipient: _id,
